...
# Subject: Validated SF-36 syntax based on Standardized US population weights
# Explanation:
...
sf36 <- function( X = NULL ) {
  ...
  ## END;
  X[, rpa] <- lapply(X[, rpa], outRangeNA, Max = 2L) #check whether raw scores are 1 and 2, instead of 0 and 1
  ...
  ## END;
  X[, RM] <- lapply(X[, RM], outRangeNA, Max = 2L) #check whether raw scores are 1 and 2, instead of 0 and 1
